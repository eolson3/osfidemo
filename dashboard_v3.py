...
# Pages
# -----------------------------
def render_branding(summary_row: pd.Series) -> None:
    # Branding lives on the summary row in this demo dataset.
    ...
    # Streamlit's st.image does not reliably render remote SVGs. We handle SVGs via HTML <object>,
    # and fall back gracefully if the logo can't be loaded.
    logo_html = ""
    if logo_url:
        if logo_url.lower().endswith(".svg"):
            # <object> renders SVG more consistently than <img> in Streamlit markdown.
            logo_html = f'''
            <div class="osfi-logo">
              <object data="{logo_url}" type="image/svg+xml" width="44" height="44">
                <img src="{logo_url}" width="44" height="44" alt="logo" />
              </object>
            </div>
            '''
        else:
            logo_html = f'<img class="osfi-logo-img" src="{logo_url}" width="44" height="44" alt="logo" />'

    st.markdown(
        f"""
        <div class="osfi-brand">
          <div class="osfi-brand-left">
            {logo_html}
            <div class="osfi-brand-text">
              <div class="osfi-inst-name">{name}</div>
              <div class="osfi-report">Institutions Dashboard (Demo){(' • Report month: ' + report_month) if report_month else ''}</div>
            </div>
          </div>
        </div>
        """,
...
def render_column_customizer(
    page_key: str,
    all_columns: list[str],
    default_columns: list[str],
    label: str = "Customize",
) -> list[str]:
    """Checkbox-based column selector.

    Uses session_state key f"{page_key}_columns".
    """
    state_key = f"{page_key}_columns"
    if state_key not in st.session_state:
        st.session_state[state_key] = list(default_columns)

    # Ensure stable ordering and no unknowns
    current = [c for c in st.session_state[state_key] if c in all_columns]
    if not current:
    ...
    with st.expander(label, expanded=False):
        ...
        with c2:
            if st.button("Select none", key=f"{page_key}_sel_none"):
                current = []
        with c3:
            query = st.text_input("Search columns", key=f"{page_key}_col_search", placeholder="Type to filter…")

        # Render checkboxes (optionally filtered)
        q = (st.session_state.get(f"{page_key}_col_search") or "").strip().lower()
        visible = [c for c in all_columns if (q in c.lower())] if q else list(all_columns)

        # Compact checkbox list
        for col in visible:
            checked = col in current
            new_checked = st.checkbox(col, value=checked, key=f"{page_key}_col_{col}")
            if new_checked and col not in current:
                current.append(col)
            if (not new_checked) and col in current:
                current.remove(col)
...
def render_entity_tab(df_all: pd.DataFrame, row_type: str, title: str, page_key: str):
    ...
    with c_custom:
        ...
        default_cols = {
            "project": ["name_or_title","osf_link","created_date","modified_date","storage_region","storage_gb","license","resource_type","views_last_30_days","downloads_last_30_days","report_yearmonth"],
            "registration": ["name_or_title","osf_link","created_date","modified_date","storage_region","storage_gb","license","resource_type","views_last_30_days","downloads_last_30_days","report_yearmonth"],
            "preprint": ["name_or_title","osf_link","created_date","modified_date","doi","storage_region","storage_gb","license","resource_type","views_last_30_days","downloads_last_30_days","report_yearmonth"],
        }
        default_visible = [c for c in default_cols.get(row_type, df.columns.tolist()) if c in df.columns]
        selected_cols = render_column_customizer(
            page_key=page_key,
            all_columns=df.columns.tolist(),
            default_columns=default_visible,
            label="Customize",
        )
    with c_dl:
        st.download_button(
